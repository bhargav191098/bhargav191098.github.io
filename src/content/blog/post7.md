...
Let's take the system diagram from the initial blog.

![SD](/SD-ModelEndpoint.png)

We are going to deploy the model as an endpoint. We'll be touching upon #1,#2 & #3 in the diagram above.

First, I'm going to use llama3.1 8B variant. This makes inference with bf16 precision possible on a 4090 GPU card. Second, I want to start using vLLM from this layer itself. This would be helpful in understanding what that library abstracts away later. For the GPU renting option, I'm gonna stick to Runpod VMs for now. I'm going to wrap the model around a fast-api endpoint. Incrementally, and ideally I'd love to benchmark the performance against Huggingface TGI once I implement the streaming tokens and continous batching of requests in the project.
<br>
<br>
...
If you have stuck to reading this long procedural you might be aware of how incrementally tokens are generated by Llama. I'll bring back the Llama arch diagram that <a href="https://www.youtube.com/watch?v=oM4VmoabDAI">Umar Jamil</a> used.
...
We introduce the term temperature T in the exponential term and here is a visual representation of how the distribution shifts.

![gifOfDistributionShift](/tempScaling.gif)

As you can see when the temperature increases, the distribution is somewhat smoothened, leading to a possibility of less-likely (from the perspective of softmax) being picked as the next possible word.

The second way to relax is to pick tokens differently! Introducing top-p. Effectively, you pick a token from a subset of tokens whos probability add up to a p%. Algorithmically, here's what you can do : <br>
